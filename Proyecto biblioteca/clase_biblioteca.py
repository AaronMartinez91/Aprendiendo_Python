...
class Biblioteca:
    ...
    def prestar(self, libro, lector):
        ...
        if isinstance(libro, LibroIncunable):
            mensaje = mensaje_base + f" {lector.nombre} {lector.apellido1} jamás podrá sacar {libro.titulo} porque es un incunable."
        ...
        if lector.fecha_posible_para_préstamo > fecha_actual:
            ...
            mensaje = mensaje_base + f"NO se presta el libro {libro.titulo} a {lector.nombre} {lector.apellido1} porque le quedan {dias_penalizacion} días de penalización."
            self.registrar_en_diario(mensaje)
            return False
        ...
        if libro_prestado:
            mensaje = mensaje_base + f"{lector.nombre} {lector.apellido1} no puede sacar el libro '{libro.titulo}' porque ya tiene un libro prestado."
            self.registrar_en_diario(mensaje)
            return False
